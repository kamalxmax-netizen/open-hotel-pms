...
function asBangkokDateParts(value: string): { date: string; time: string } {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const shaped: AuditRow[] = ((data ?? []) as any[]).map((row) => {
    ...
    const filtered = applySearch(grouped, queryData.search).slice(0, MAX_EXPORT_ROWS);
    ...
    for (const row of filtered) {
      const bangkok = asBangkokDateParts(row.created_at);
      lines.push(
        [
          bangkok.date,
          bangkok.time,
    ...
    const filename = `audit-export-${dateFrom}_to_${dateTo}.csv`;
    ...
    console.error("api/audit/export GET failed", err);
