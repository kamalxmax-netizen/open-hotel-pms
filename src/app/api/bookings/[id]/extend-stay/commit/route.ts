...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (source === "ota") {
      const otaResult = await commitOtaExtendOrchestrator({
      ...
      return NextResponse.json({
        success: Boolean(otaResult.success),
        reservation_id: reservationId,
        extension_reservation_id: otaResult.extension_reservation_id ?? null,
        mode: "linked_extension",
        executed_actions: otaResult.executed_actions ?? [],
        failed_actions: otaResult.failed_actions ?? [],
        pending_fix_action: otaResult.pending_fix_action ?? null,
        warnings: otaResult.warnings ?? [],
        final_price_summary: otaResult.final_price_summary ?? {},
        ota_platform_update_required: Boolean(otaResult.ota_platform_update_required),
      });
    }

    const result = await commitExtendStay({
      supabase: supabase as any,
      input: {
        reservationId,
        newCheckoutDate: parsedBody.data.new_checkout_date,
