...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Owner must be able to view the Calendar / Board.
    // Keep DEFAULT_DENY_ROLES unchanged globally.
    ...
    const [
      ...
      supabase
        .from("rooms")
        .select(
          "id, room_type_id, room_number, is_sellable, closure_reason, is_dayuse, floor_number, wing, sort_order, room_types(name_en, code)"
        )
      ...
      supabase
        ...
        .select(
          "id, reservation_id, start_date, end_date, from_room_id_snapshot, to_room_id, to_room_type_id, move_reason, pricing_policy, do_not_move, status, reservations!inner(id, status)"
        )
        .eq("status", "planned")
    ...
    if (plannedMovesError) {
      return NextResponse.json(
        { error: plannedMovesError.message },
        { status: 500 }
      );
    }
    ...
    const nights = (rawNights ?? []).filter((row: any) => {
      ...
      return (
        Boolean(reservation?.is_dayuse) &&
        String(reservation?.status ?? "") === "checked_out"
      );
    });
    ...
    if (plannedMoveReservationIds.length > 0) {
      const [
        { data: plannedReservations, error: plannedReservationsError },
        { data: plannedNights, error: plannedNightsError },
      ] = await Promise.all([
        supabase
          .from("reservations")
          .select("id, status")
      ...
      if (plannedReservationsError) {
        return NextResponse.json(
          { error: plannedReservationsError.message },
          { status: 500 }
        );
      }

      if (plannedNightsError) {
        return NextResponse.json(
          { error: plannedNightsError.message },
          { status: 500 }
        );
      }
    ...
    // ── Batch 2: Queries that depend on nights data ──
    const [groupsResult, alertsResult] = await Promise.all([
      ...
      reservationIds.length > 0
        ? supabase
            ...
            .select(
              "id, reservation_id, alert_code, alert_template_id, note, custom_message, display_surfaces, severity, is_dismissed, created_at, created_by, alert_codes(code, description, dept, auto_on_co, icon), alert_templates(id, code, name, description, category, display_surfaces, severity, icon)"
            )
    ...
    const alertSummaryByReservationId = new Map<
      string,
      {
        count: number;
...

    {
      ...
      const legacyCodes = Array.from(
        new Set(
          reservationAlerts
            .filter(
              (row: any) =>
                !row?.alert_template_id && row?.alert_code
            )
            .map((row: any) =>
              normalizeAlertCodeKey(row.alert_code)
            )
            .filter(Boolean)
        )
      );
      ...
      if (legacyCodes.length > 0) {
        const {
          ...
          .select(
            "id, code, name, description, category, display_surfaces, severity, icon"
          );
        ...
        templateMap = new Map(
          (templates ?? []).map((template: any) => [
            normalizeAlertCodeKey(template.code),
            template,
          ])
        );
      }

      const resolvedRows = attachTemplateFallback(
        reservationAlerts,
        templateMap
      );
      ...
      grouped.forEach((rows, reservationId) => {
        const alerts = rows
          .map((row) =>
            mapEffectiveReservationAlert(row)
          )
          .filter((alert) => !alert.is_dismissed);

        const visibleAlerts = filterAlertsForSurface(
          alerts,
          "calendar"
        );
        ...
        if (summary.count > 0) {
          alertSummaryByReservationId.set(
            reservationId,
            summary
          );
        }
    ...
    const nightRoomByReservationDate = new Map<
      string,
      string
    >();

    for (const row of nights ?? []) {
      const reservationRef = (row as any)?.reservations as {
        id?: string | null;
        status?: string | null;
      } | null;
      ...
      const reservationId = reservationRef.id
        ? String(reservationRef.id)
        : "";

      const stayDate = String(
        (row as any)?.stay_date ?? ""
      );
      ...
      if (!reservationId || !stayDate || !roomId) {
        continue;
      }

      nightRoomByReservationDate.set(
        `${reservationId}::${stayDate}`,
        roomId
      );
    }

    const visiblePlannedMoves = plannedMoveRows.filter(
      (row: any) => {
        const reservationId = String(
          row?.reservation_id ?? ""
        );
        ...
        if (
          !reservationId ||
          !activePlannedReservationIds.has(
            reservationId
          )
        ) {
          return false;
        ...
        const sourceRoomId = String(
          row?.from_room_id_snapshot ?? ""
        );

        if (!sourceRoomId) return true;

        const moveStartDate = String(
          row?.start_date ?? ""
        );

        const moveEndDate = String(
          row?.end_date ?? ""
        );
        ...
        // Keep future planned moves visible.
        if (moveStartDate > today) {
          return true;
        }

        // For moves effective today, hide stale rows
        // whose source room no longer matches actual room.
        ...
        const assignedRoomId =
          nightRoomByReservationDate.get(
            `${reservationId}::${today}`
          );
    ...
    // ── Batch 3: Planned move metadata ──
    const [
      plannedReservationsResult,
      plannedRoomsResult,
    ] = await Promise.all([
      plannedReservationIds.length > 0
        ? supabase
            .from("reservations")
            .select(
              "id, booking_code, guest_name, checkin_date, checkout_date, booking_group_id"
            )
    ...
    if (!plannedReservationsResult.error) {
      (plannedReservationsResult.data ?? []).forEach(
        (row: any) => {
          reservationMetaById.set(String(row.id), {
            booking_code:
              row.booking_code ?? null,
            guest_name:
              row.guest_name ?? null,
            checkin_date:
    ...
    const plannedRoomNumberById = new Map<
      string,
      string
    >();

    if (!plannedRoomsResult.error) {
      (plannedRoomsResult.data ?? []).forEach(
        (row: any) => {
          plannedRoomNumberById.set(
            String(row.id),
    ...
    // Build room_type_id → name lookup from rooms data
    ...
    const perNightRoomsByReservation =
    ...
    for (const nightRow of nights ?? []) {
      const reservationRef =
        (nightRow as any)?.reservations as {
          id?: string;
          parent_reservation_id?: string | null;
        } | null;

      const parentReservationId =
        reservationRef?.parent_reservation_id
          ? String(
              reservationRef.parent_reservation_id
            )
          : null;
    ...
    for (const n of nights ?? []) {
      ...
      if (resId && roomId) {
        const nightToRoom =
          perNightRoomsByReservation.get(
            resId
          ) ??
        ...
        perNightRoomsByReservation.set(
          resId,
          nightToRoom
        );
      }
      ...
      if (!seen.has(key)) {
        ...
        const linkedRootId =
          parentReservationId ??
          (linkedRootIds.has(String(res.id))
            ? String(res.id)
            : null);
        ...
        const entry = {
          ...
          group_code:
            groupMeta?.group_code ?? null,
          group_name:
            groupMeta?.group_name ?? null,
          ...
          room_type_id: n.room_type_id
            ? String(n.room_type_id)
            : "",
          room_type: n.room_type_id
            ? roomTypeNameById.get(
                String(n.room_type_id)
              ) ?? ""
            : "",
          do_not_move: Boolean(
            res.do_not_move_assigned_room
          ),
          alert_count:
            alertSummaryByReservationId.get(
              String(res.id)
            )?.count ?? 0,
          first_alert_message:
            alertSummaryByReservationId.get(
    ...
    // Build HK status from pre-fetched data
    const hkStatusByRoomId = new Map<
      string,
      string
    >();
    ...
    const responseEntries: Array<{
      ...
      linked_root_id: string | null;
      linked_reservation_ids?: string[];
      per_night_rooms?: Record<
        string,
        string
      >;
    }> = [];

    for (const roomReservations of Object.values(
      resMap
    )) {
    ...
    responseEntries.push(
      ...Array.from(unassignedMap.values())
    );

    const linkedRootIdsForResponse =
      Array.from(
        new Set(
          responseEntries
            .map((entry) =>
              entry.linked_root_id
                ? String(
                    entry.linked_root_id
                  )
                : ""
    ...
    const linkedIdsByRoot = new Map<
      string,
      Set<string>
    >();

    if (
      linkedRootIdsForResponse.length > 0
    ) {
      const {
        data: linkedRoots,
        error: linkedRootsError,
      } = await supabase
        .from("reservations")
        .select("id")
        .in(
          "id",
          linkedRootIdsForResponse
        );
      ...
      if (linkedChildrenError) {
        return NextResponse.json(
          {
            error:
              linkedChildrenError.message,
          },
          { status: 500 }
        );
      }
    ...
    for (const entry of responseEntries) {
      ...
      entry.linked_reservation_ids =
        Array.from(
          linkedSet.values()
        ).sort();
    }

    const splitPerNightRoomsByReservation =
      new Map<
        string,
    ...
    for (
      const [
        reservationId,
        nightToRoom,
      ] of perNightRoomsByReservation.entries()
    ) {
    ...
    for (const entry of responseEntries) {
      const splitMap =
        splitPerNightRoomsByReservation.get(
          entry.reservation_id
        );

      if (splitMap) {
        entry.per_night_rooms =
          splitMap;
      }
    }

    // Build final rooms list with reservations
    const data = (rooms ?? []).map(
      (room) => {
        const rt =
          room.room_types as unknown as {
            name_en: string;
            code: string;
          } | null;
        ...
        return {
          ...
          is_sellable:
            room.is_sellable,
          is_dayuse:
            room.is_dayuse ?? false,
          closure_reason:
            room.closure_reason,
          hk_status:
            hkStatusByRoomId.get(
              String(room.id)
            ) ?? null,
          reservations:
            resMap[room.id] ?? [],
    ...
    return NextResponse.json({
      ...
      blocks: blocks ?? [],
      planned_moves:
        visiblePlannedMoves.map(
          (row: any) => {
            const reservationMeta =
              reservationMetaById.get(
                String(
                  row.reservation_id
                )
              );
            ...
            const groupId =
              reservationMeta
                ?.booking_group_id
                ? String(
                    reservationMeta.booking_group_id
                  )
                : null;
            ...
            return {
              ...
              booking_code:
                reservationMeta?.booking_code ??
                null,
              guest_name:
                reservationMeta?.guest_name ??
                null,
              checkin_date:
                reservationMeta?.checkin_date ??
                null,
              checkout_date:
                reservationMeta?.checkout_date ??
                null,
              booking_group_id:
                groupId,
              group_code:
              ...
              move_reason:
                row.move_reason ?? null,
              pricing_policy:
                row.pricing_policy ??
                "keep_rtc",
              do_not_move:
                Boolean(row.do_not_move),
              status:
                String(
                  row.status ??
                    "planned"
