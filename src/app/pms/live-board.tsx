...
function RoomCard({
  ...
  return (
    ...
    >
      ...
      <p className="mt-1.5 truncate text-[11px] font-semibold text-slate-600">{roomType}</p>
      {!sellable && (
        <div className="mt-1.5 flex items-center justify-between gap-2">
          <span className="rounded-md bg-rose-700 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white">
            Not For Sale
          </span>
          {note ? <span className="truncate text-[10px] text-slate-600">{note}</span> : null}
        </div>
      )}
    </button>
  );
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-700">{lane.floorLabel}</p>
        <p className="text-xs text-slate-500">{lane.laneLabel}</p>
      </div>
      ...
      {lane.showCorridorBelow ? (
        <div className="rounded-lg border border-slate-400/60 bg-slate-300 py-4 text-center text-xs font-semibold uppercase tracking-wider text-slate-700">
          Corridor
        </div>
      ) : null}
    </div>
...
function FloorGap() {
  return (
    <div className="my-5 border-t-2 border-red-500/70 pt-4">
      <div className="rounded-md border border-slate-400/60 bg-slate-300 py-3 text-center text-[11px] font-semibold uppercase tracking-wider text-slate-700">
        Floor Separation
      </div>
    </div>
  );
...
export default function LiveBoard({ refreshToken }: { refreshToken: number }) {
  ...
  return (
    <section className="space-y-5">
      <div className="card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Room Plan (Sheet-style Card Board)</h1>
            <p className="mt-1 text-sm text-slate-600">
              Live board from database. Select a stay date to see booking/housekeeping status on each room card.
            </p>
            <p className="mt-1 text-xs text-slate-500">Tip: click a room card to load guest detail for the selected date.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <label className="text-xs font-medium text-slate-600">Board Date</label>
            <input
              type="date"
              value={selectedDate}
      ...
      <div className="card p-5">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-700">Status Legend</h2>
        <div className="mt-3 flex flex-wrap gap-2">
          {statusLegend.map((item) => (
            <span key={item.label} className="badge bg-slate-100 text-slate-700">
      ...
      <div className="card overflow-x-auto p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <p className="text-xs text-slate-500">Floor order: 2 → 3 → 1</p>
        </div>

        <div className="min-w-[1200px] rounded-2xl border border-slate-200 bg-slate-100/60 p-4">
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <p className="text-xs text-slate-500">Room {selectedRoomNumber}</p>
                <button
                  type="button"
                  onClick={closeDetailModal}
            ...
            {detailLoading ? <p className="text-sm text-slate-600">Loading reservation detail...</p> : null}
            {!detailLoading && detailError ? (
              <p className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">{detailError}</p>
            ) : null}
            {!detailLoading && !detailError && !detail ? (
              <p className="text-sm text-slate-600">No detail for this room/date.</p>
            ) : null}
