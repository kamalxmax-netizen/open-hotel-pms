...
export default function LinenRatesPage() {
  ...
  // Initialize local rates when data loads
  React.useEffect(() => {
    if (rates) {
      const initial: Record<number, number> = {};
      rates.forEach(r => {
        initial[r.linen_item_id] = r.rate_per_piece;
      });
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto pb-20 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 uppercase tracking-tight">Setting — ราคา Vendor</h1>
        ...
        <div className="py-4">
          {isLoading ? (
            ...
            <div className="space-y-3">
              {history.map((h) => (
                <div key={h.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-800">
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">เริ่มใช้เมื่อ</p>
                    <p className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase">{h.effective_month}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-black text-[#1B4038] dark:text-emerald-400">{h.rate_per_piece.toFixed(2)} ฿</p>
                    <p className="text-[10px] text-slate-400">{h.note || "ไม่มีบันทึก"}</p>
                  </div>
                </div>
              ))}
            </div>
