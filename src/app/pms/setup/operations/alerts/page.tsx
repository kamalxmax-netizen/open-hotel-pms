    ...
    <div className="max-w-6xl space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 mt-0.5">Alert Templates</h1>
          <p className="text-sm text-slate-500 mt-1">Configure reusable alert templates, severity, and display surfaces.</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={saveForm} className="rounded-2xl border border-brand-200 bg-brand-50 p-5 space-y-4">
          <h2 className="text-base font-bold text-slate-900">{editing ? "Edit Alert Template" : "New Alert Template"}</h2>
          ...
          <div>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {SURFACES.map((surface) => (
                <label key={surface.value} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="text-sm text-slate-600">{(item.display_surfaces ?? []).join(", ")}</td>
