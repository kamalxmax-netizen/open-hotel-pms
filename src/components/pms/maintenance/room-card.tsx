...
export function MaintenanceRoomCard({ room, onClick }: RoomCardProps) {
    ...
    return (
        <div
            className={`bg-[var(--bg-surface)] rounded-lg border shadow-sm hover:shadow-md transition-all cursor-pointer ${
                ...
                <div className="flex justify-between items-start">
                    <div>
                        <div className="flex items-center gap-1.5">
                            <span className="text-lg font-bold text-[var(--text-primary)]">{room.room_number}</span>
                            <span className="text-[10px] font-medium text-[var(--text-muted)] bg-[var(--bg-surface-hover)] px-1.5 py-0.5 rounded">
                                {room.room_type_code}
                            </span>
                        </div>
