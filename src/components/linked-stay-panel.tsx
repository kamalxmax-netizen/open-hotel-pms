...
export default function LinkedStayPanel({ linkedStay, currentReservationId, onSwitchTab }: LinkedStayPanelProps) {
  ...
  return (
    <div className="bg-indigo-50/40 border border-indigo-100 rounded-xl p-4 mb-6 shadow-sm dark:bg-indigo-500/5 dark:border-indigo-500/20">
      <div className="flex justify-between items-center mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-indigo-700 bg-indigo-100/50 px-2 py-0.5 rounded font-medium dark:bg-indigo-500/15 dark:text-indigo-300">
            {fdCi} → {fdCo} ({linkedStay.full_nights} night{linkedStay.full_nights !== 1 ? "s" : ""})
          </span>
        </div>
        <div className="text-xs text-indigo-700 font-bold dark:text-indigo-300">
        ...
        {linkedStay.segments.map((seg, idx) => {
          ...
          return (
            <div key={seg.reservation_id} className="flex items-center gap-2">
              ...
              {/* Add an arrow pointing to the next segment if not the last one */}
