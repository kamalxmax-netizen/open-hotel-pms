        ...
        <div className="overflow-y-auto p-4 flex-1">
          {roomNote && (
            <div className="mb-4 rounded-lg border border-sky-200 bg-sky-50 px-3 py-2">
              <p className="text-xs font-bold text-sky-800">No Service Note</p>
              <p className="mt-1 text-xs text-sky-700 whitespace-pre-wrap">{roomNote}</p>
            </div>
          )}
          ...
          {maintenanceChecklist.length > 0 && (
            <div className="mt-5 space-y-3">
              <div className="rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2">
                <p className="text-xs font-bold text-indigo-800">Special Maintenance Tasks</p>
          ...
          {hkTraces && hkTraces.length > 0 && (
            <div className="mt-5 space-y-3">
              <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
                <p className="text-xs font-bold text-amber-800">HK Trace Reminders</p>
                <p className="text-[11px] text-amber-700">Please follow these room instructions while cleaning.</p>
              </div>
          ...
          {loanCollections && loanCollections.length > 0 && (
            <div className="mt-5 space-y-3">
              <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
                <p className="text-xs font-bold text-amber-800">Items to Collect (Loan Returns)</p>
                <p className="text-[11px] text-amber-700">Please collect these items from the guest or room.</p>
              </div>
              <div className="space-y-2">
          ...
          <button
            onClick={() => {
              if (loanCollections && loanCollections.length > 0) {
                const allLoansChecked = loanChecks.every(Boolean);
                if (!allLoansChecked) {
              ...
              const collectedIds = loanCollections
                ? loanCollections.filter((_, i) => loanChecks[i]).map(l => l.trace_id)
                : [];
