        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 overflow-y-auto space-y-4 flex-1">
                    {results.length === 0 ? (
                        <p className="text-sm text-slate-500 text-center py-8">No arrivals needed auto-assignment.</p>
                    ) : (
                        results.map(r => (
                            <div key={r.reservation_id} className={`rounded-xl border p-4 ${r.status !== 'success' ? 'border-rose-200 bg-rose-50/30' : 'border-slate-200 bg-white'}`}>
                                <div className="flex justify-between items-start mb-2">
                                    <div>
                                        <h3 className="font-semibold text-slate-900">{r.guest}</h3>
                                ...
                                {r.status === "success" && r.breakdown && (
                                    <div className="mt-3 grid grid-cols-2 lg:grid-cols-4 gap-2 text-xs">
                                        <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                            <div className="text-slate-500 mb-0.5">Preferences</div>
                                            <div className="font-medium text-slate-700">{r.breakdown.preference ?? 0} pts</div>
                                        </div>
                                        <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                            <div className="text-slate-500 mb-0.5">Bed Capacity</div>
                                            <div className="font-medium text-slate-700">{r.breakdown.bed ?? 0} pts</div>
                                        </div>
                                        <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                            <div className="text-slate-500 mb-0.5">Quality & HK</div>
                                            <div className="font-medium text-slate-700">{(r.breakdown.quality ?? 0) + (r.breakdown.hk ?? 0)} pts</div>
                                        </div>
                                        <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                            <div className="text-slate-500 mb-0.5">Proximity Bonus</div>
                                ...
                                {r.reasons && r.reasons.length > 0 && (
                                    <div className="mt-3 bg-slate-50 p-3 rounded-lg border border-slate-100 text-xs text-slate-600 space-y-1">
                                        {r.reasons.map((rsn, i) => (
                                            <div key={i}>{rsn}</div>
                                        ))}
                                    </div>
