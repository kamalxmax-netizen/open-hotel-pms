...
export default function ConfirmationLetter({ reservationId, onClose }: ConfirmationLetterProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 p-4 overflow-auto">
      <div className="mx-auto max-w-4xl bg-white rounded-xl shadow-xl">
        <div className="no-print flex items-center justify-between border-b border-slate-200 px-5 py-3">
          <h2 className="text-base font-bold text-slate-900">Booking Confirmation</h2>
        ...
        {loading && (
          <div className="p-10 text-center text-slate-500">Loading confirmation...</div>
        )}
        ...
        {!loading && data && (
          <div className="print-container p-8 text-slate-900">
            <header className="border-b border-slate-200 pb-4 mb-6">
              <h1 className="text-2xl font-extrabold tracking-tight">{data.hotel_name}</h1>
              <p className="text-sm text-slate-600 mt-1">{data.hotel_address}</p>
            ...
            <section className="mb-6">
              ...
              <div className="mt-2 grid grid-cols-2 gap-4 text-sm">
                <p><span className="text-slate-500">Confirmation #:</span> <strong>{data.confirmation_number}</strong></p>
                <p><span className="text-slate-500">Date:</span> <strong>{generatedDate}</strong></p>
              </div>
            </section>

            <section className="mb-6">
              <h3 className="text-sm font-bold uppercase tracking-wide text-slate-700 mb-2">Guest Details</h3>
              <table className="w-full border border-slate-200 text-sm">
                <tbody>
                  <tr className="border-b border-slate-200">
                    <td className="w-48 bg-slate-50 px-3 py-2 font-semibold">Guest Name</td>
                    <td className="px-3 py-2">{data.guest_name}</td>
                  </tr>
                  <tr>
                    <td className="bg-slate-50 px-3 py-2 font-semibold">Phone</td>
            ...
            <section className="mb-6">
              <h3 className="text-sm font-bold uppercase tracking-wide text-slate-700 mb-2">Stay Details</h3>
              <table className="w-full border border-slate-200 text-sm">
                <tbody>
                  <tr className="border-b border-slate-200">
                    <td className="w-48 bg-slate-50 px-3 py-2 font-semibold">Room Type</td>
                    <td className="px-3 py-2">{data.room_type}</td>
                  </tr>
                  <tr className="border-b border-slate-200">
                    <td className="bg-slate-50 px-3 py-2 font-semibold">Room Number</td>
                    <td className="px-3 py-2">{data.room_number}</td>
                  </tr>
                  <tr className="border-b border-slate-200">
                    <td className="bg-slate-50 px-3 py-2 font-semibold">Check-in</td>
                    <td className="px-3 py-2">{fmtDate(data.checkin_date)}</td>
                  </tr>
                  <tr className="border-b border-slate-200">
                    <td className="bg-slate-50 px-3 py-2 font-semibold">Check-out</td>
                    <td className="px-3 py-2">{fmtDate(data.checkout_date)}</td>
                  </tr>
                  <tr>
                    <td className="bg-slate-50 px-3 py-2 font-semibold">Nights</td>
            ...
            <section className="mb-8">
              <h3 className="text-sm font-bold uppercase tracking-wide text-slate-700 mb-2">Rate Breakdown</h3>
              <table className="w-full border border-slate-200 text-sm">
                <tbody>
                  <tr className="border-b border-slate-200">
                    <td className="w-48 bg-slate-50 px-3 py-2 font-semibold">Rate / Night</td>
                    <td className="px-3 py-2 text-right">฿ {fmtMoney(data.rate_per_night)}</td>
                  </tr>
                  <tr className="border-b border-slate-200">
                    <td className="bg-slate-50 px-3 py-2 font-semibold">Total</td>
                    <td className="px-3 py-2 text-right">฿ {fmtMoney(data.total_price)}</td>
                  </tr>
                  <tr className="border-b border-slate-200">
                    <td className="bg-slate-50 px-3 py-2 font-semibold">Deposit</td>
                    <td className="px-3 py-2 text-right">฿ {fmtMoney(data.deposit_amount)}</td>
                  </tr>
                  <tr>
                    <td className="bg-slate-50 px-3 py-2 font-semibold">Room Balance Due</td>
            ...
            <section className="mb-8">
              <h3 className="text-sm font-bold uppercase tracking-wide text-slate-700 mb-2">Terms & Conditions</h3>
              <div className="text-xs leading-6 text-slate-700 border border-slate-200 bg-slate-50 p-3">
            ...
            <footer className="pt-6 border-t border-slate-200">
              <div className="grid grid-cols-2 gap-8 text-sm">
                ...
                <div>
                  <p className="text-slate-500 mb-12">Front Desk</p>
